...
const paginate = async (repository, req, limit, name) => {
  ...
  const { count, rows } = !name
    ? await repository(offset, limit)
    : await repository(offset, limit, name)
  ...
  return {
    ...
    previousPagesUrl: previousPage,
    nexPagesUrl: nextPage
  }
}
module.exports = {
  paginate
